...
import com.wjf.self_demo.databinding.ActivityBehaviorBinding
import com.wjf.self_demo.databinding.HolderColorCubeBinding
import com.wjf.self_demo.nestedrecycler.ColorCubeEntity
import com.wjf.self_demo.nestedrecycler.ColorCubeViewHolder
import org.jxxy.debug.corekit.common.BaseActivity
import org.jxxy.debug.corekit.recyclerview.SingleTypeAdapter

class BehaviorActivity : BaseActivity<ActivityBehaviorBinding>() {
    private val adapter by lazy { ListAdapter() }
    override fun bindLayout(): ActivityBehaviorBinding {
        return ActivityBehaviorBinding.inflate(layoutInflater)
    }
    ...
    override fun initView() {
        ...
        val data = mutableListOf<ColorCubeEntity>()
        for (i in 0..3) {
            data.add(ColorCubeEntity(Color.parseColor("#9ABB86FC")))
            data.add(ColorCubeEntity(Color.parseColor("#9A03DAC5")))
            data.add(ColorCubeEntity(Color.parseColor("#9A6200EE")))
            data.add(ColorCubeEntity(Color.parseColor("#9A018786")))
            data.add(ColorCubeEntity(Color.parseColor("#9ABB86FC")))
            data.add(ColorCubeEntity(Color.parseColor("#9A03DAC5")))
            data.add(ColorCubeEntity(Color.parseColor("#9A6200EE")))
            data.add(ColorCubeEntity(Color.parseColor("#9A018786")))
    ...
    class ListAdapter : SingleTypeAdapter<ColorCubeEntity>() {
        override fun createViewHolder(viewType: Int, inflater: LayoutInflater, parent: ViewGroup): RecyclerView.ViewHolder {
            return ColorCubeViewHolder(HolderColorCubeBinding.inflate(inflater, parent, false))
        }
    }
}

class TitleBarBehavior(context: Context, attributeSet: AttributeSet?) : CoordinatorLayout.Behavior<TextView>(context, attributeSet) {

    // child是自己，dependency是协同者
    override fun layoutDependsOn(parent: CoordinatorLayout, child: TextView, dependency: View): Boolean {
        return ((dependency.layoutParams as? CoordinatorLayout.LayoutParams)?.behavior as? BottomSheetBehavior) != null
    }

    override fun onDependentViewChanged(parent: CoordinatorLayout, child: TextView, dependency: View): Boolean {
        ((dependency.layoutParams as? CoordinatorLayout.LayoutParams)?.behavior as? BottomSheetBehavior)?.let {
//            Log.d("wjftc", "状态${it.state} ")
            /*when (it.state){
                BottomSheetBehavior.STATE_DRAGGING-> {
                    Log.d("wjftc", "拖动中")
                    child.setBackgroundColor(ResourceUtil.getColor(R.color.color_green))
                }
                BottomSheetBehavior.STATE_SETTLING-> {
                    Log.d("wjftc", "惯性滑动")
                    child.setBackgroundColor(ResourceUtil.getColor(R.color.color_yellow))
                }
            }*/
            return true
        }
